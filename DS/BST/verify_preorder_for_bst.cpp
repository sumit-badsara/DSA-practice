...
class BST{
    ...
    public:
        ...
        int constructPreorder(vector<int> pre)
        {
            int index = 0;
...
int main()
{
    /*
        ...
        Worst Alternative:
            ...
            Simply take current node, divide the remaining array in two parts,
            less than current and more than current, if not possible return false.
            Otherwise, recurse for both subarrays.
    */

    vector <int> pre({40, 30, 35, 80, 90, 100});
    BST* newBst = new BST;

    //Construct BSt from Preorder
    int buildSize = newBst->constructPreorder(pre);

    if(buildSize < pre.size())
        cout<<"Not possible! Incorrent preorder for BST.\n";
    else
    {
        cout<<"Possible! BST build successful.\nInorder: ";
